...
static int vring_add_sgs(struct virtqueue *_vq,
		      struct scatterlist *sgs[],
		      unsigned int out_sgs,
		      unsigned int in_sgs,
		      void *data,
...
static int vring_add_outbuf(struct virtqueue *vq,
...
static int vring_add_inbuf(struct virtqueue *vq,
			struct scatterlist sg[], unsigned int num,
			void *data,
			gfp_t gfp)
{
...
static unsigned int vring_get_vring_size(struct virtqueue *_vq)
{
...
static struct virtqueue_ops vring_ops = {
	...
	.add_sgs		= vring_add_sgs,
	.add_outbuf		= vring_add_outbuf,
	.add_inbuf		= vring_add_inbuf,
