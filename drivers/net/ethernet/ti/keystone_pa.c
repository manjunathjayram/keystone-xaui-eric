...
static int skb_sctp_csum_help(struct sk_buff *skb)
{
	__wsum csum;
	__le32 *csum_ptr;
	int ret = 0, start_offset, csum_offset;
	...
	BUG_ON(start_offset >= skb_headlen(skb));

	csum_offset = start_offset + skb->csum_offset;
	BUG_ON(csum_offset + sizeof(__le32) > skb_headlen(skb));

	if (skb_cloned(skb) &&
	    !skb_clone_writable(skb, csum_offset + sizeof(__le32))) {
		ret = pskb_expand_head(skb, 0, 0, GFP_ATOMIC);
		if (ret)
			goto out;
	}

	csum_ptr = (__le32 *)(skb->data + csum_offset);
	*csum_ptr = 0;
	csum = __skb_checksum(skb, start_offset, skb->len - start_offset,
				~(__u32)0, &ops);
	*csum_ptr = cpu_to_le32(~csum);
